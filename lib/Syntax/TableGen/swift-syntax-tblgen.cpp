...
static void printFileHeader(raw_ostream &OS, StringRef Filename) {
  OS << "//===----------------------------------------------------------------------===//\n"
        "//\n"
...
#include "swift/Syntax/SyntaxKinds.def"
    ...
    OS << "public enum SyntaxKind: String, Codable {\n";
    for (auto name : names) {
      OS << "  case " << sanitized(name) << " = \"" << name << "\"\n";
    }
  ...
  void printSwiftTokenKindEnum() {
    ...
    ArrayRef<std::pair<const char *, const char *>> vals = {
      { "eof", "eof" },
...
#include "swift/Syntax/TokenKinds.def"
    ...
    OS << "public enum TokenKind: Codable {\n";
    ...
    for (auto pair : vals) {
      auto text = escaped(pair.second);
      OS << "    case ." << pair.first << ": return \"" << text << "\"\n";
    }
    OS << "    }\n"
    "  }\n\n";
    ...
    "    switch kind {\n";
    for (auto pair : vals) {
      OS << "    case \"" << pair.first << "\": self = ." << pair.first << "\n";
    }
    for (auto name : stringContaining) {
      OS << "    case \"" << name << "\":\n"
    ...
    "    switch self {\n";
    for (auto pair : vals) {
      OS << "    case ." << pair.first << ": return \"" << pair.first << "\"\n";
    }
    for (auto name : stringContaining) {
    ...
    OS << "extension TokenKind: Equatable {\n"
    "  public static func ==(lhs: TokenKind, rhs: TokenKind) -> Bool {\n"
    "    switch (lhs, rhs) {\n";
    for (auto pair : vals) {
      OS << "    case (." << pair.first << ", ."
      << pair.first << "): return true\n";
    }
    for (auto name : stringContaining) {
  ...
  virtual bool printSyntaxImplementation(const Record &Node) {
    ...
    for (auto &Child : getChildrenOf(Node)) {
      ...
      OS << "  public var " << ChildName << ": " << ChildTypeName << " {\n"
            "    return " << ChildTypeName << "(root: root, data: data.cachedChild(at: Cursor." << ChildName << "))\n"
            "  }\n\n";
      ...
      if (isToken(ChildType)) {
        auto RawNewChild = NewChildParam + ".raw";
  ...
  virtual void printTokenAssertion(std::string VariableName,
    ...
    if (!canBeAnyOfSyntaxCategory(TokenRec)) {
      OS << "(." << sanitized(Kind) << ", _, _, _)";
    }
    OS << " = " << VariableName << " else {\n"
