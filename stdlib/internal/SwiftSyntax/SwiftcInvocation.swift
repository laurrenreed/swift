...
/// Runs `which` to determine the full path to the provided binary name.
/// - Parameter toolName: The name of the tool you're trying to find, in your
///                       PATH.
/// - Returns: A file URL to the location of that executable, or `nil` if the
///            tool could not be found. 
func which(_ toolName: String) -> URL? {
  let path = URL(fileURLWithPath: "/usr/bin/which")
  let result = run(path, arguments: [toolName])
...
struct SwiftcRunner {
  ...
  static let macOSSDK: String? = {
    guard let xcrun = which("xcrun") else { return nil }
    let result = run(xcrun, arguments: ["--show-sdk-path"])
    guard result.wasSuccessful else { return nil }
    let toolPath = result.stdout.trimmingCharacters(in: .whitespacesAndNewlines)
