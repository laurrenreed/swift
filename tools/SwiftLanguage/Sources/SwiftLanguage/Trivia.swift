...
public enum TriviaPiece: Codable {
  ...
  func print<StreamType: TextOutputStream>(to stream: inout StreamType) {
    ...
    case let .tabs(count): printRepeated("\t", count: count, to: &stream)
    case let .verticalTabs(count): printRepeated("\u{2B7F}", count: count, to: &stream)
    case let .formfeeds(count): printRepeated("\u{240C}", count: count, to: &stream)
    case let .newlines(count): printRepeated("\n", count: count, to: &stream)
    case let .backticks(count): printRepeated("`", count: count, to: &stream)
...
public struct Trivia: Sequence, Codable {
  ...
  public func makeIterator() -> AnyIterator<TriviaPiece> {
    ...
    return AnyIterator {
      guard index < self.pieces.endIndex else { return nil }
      defer { index += 1 }
